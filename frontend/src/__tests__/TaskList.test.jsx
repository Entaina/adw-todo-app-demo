...
test('renders task items with drag and drop functionality', () => {
  const { container } = render(<TaskList tasks={mockTasks} onToggle={() => {}} onDelete={() => {}} onReorder={() => {}} />)
  const dragHandles = container.querySelectorAll('.drag-handle')
  expect(dragHandles).toHaveLength(2)

  const sortableItems = container.querySelectorAll('[aria-roledescription="sortable"]')
